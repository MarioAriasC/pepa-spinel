...
module Parsers
  ...
  class Parser
    ...
    def prefix_parsers(token_type)
      ...
      when Tokens::TRUE, Tokens::FALSE
        @m_parse_boolean_literal
      when Tokens::IDENT
        @m_parse_identifier
      when Tokens::BANG, Tokens::MINUS
    ...
    def infix_parsers(token_type)
      case token_type
      when Tokens::PLUS, Tokens::MINUS, Tokens::SLASH, Tokens::ASTERISK, Tokens::EQ, Tokens::NOT_EQ, Tokens::LT, Tokens::GT
    ...
    def parse_program
      ...
      Past::Program.new(statements)
    end

    private

    def parse_string_literal
      Past::StringLiteral.new(@cur_token, @cur_token.literal)
    end
    ...
    def parse_let_statement
      ...
      name = Past::Identifier.new(@cur_token, @cur_token.literal)
      return nil unless expect_peek?(Tokens::ASSIGN)

      next_token
      value = parse_expression(Precedence::LOWEST)

      value.name = name.value if value.is_a? Past::FunctionLiteral

      next_token if peek_token_is?(Tokens::SEMICOLON)
      Past::LetStatement.new(token, name, value)
    end
    ...
    def parse_expression_statement
      ...
      next_token if peek_token_is?(Tokens::SEMICOLON)
      Past::ExpressionStatement.new(token, expression)
    end
    ...
    def parse_return_statement
      ...
      Past::ReturnStatement.new(token, return_value)
    end

    def parse_integer_literal
      token = @cur_token
      value = token.literal.to_i
      Past::IntegerLiteral.new(token, value)
    end

    def parse_boolean_literal
      Past::BooleanLiteral.new(@cur_token, cur_token_is?(Tokens::TRUE))
    end

    def parse_identifier
      Past::Identifier.new(@cur_token, @cur_token.literal)
    end
    ...
    def parse_prefix_expression
      ...
      Past::PrefixExpression.new(token, operator, right)
    end
    ...
    def parse_infix_expression(left)
      ...
      Past::InfixExpression.new(token, left, operator, right)
    end
    ...
    def parse_call_expression(expression)
      ...
      Past::CallExpression.new(token, expression, arguments)
    end
    ...
    def parse_array_literal
      token = @cur_token
      Past::ArrayLiteral.new(token, parse_expression_list(Tokens::RBRACKET))
    end
    ...
    def parse_index_expression(left)
      ...
      Past::IndexExpression.new(token, left, index)
    end
    ...
    def parse_if_expression
      ...
      Past::IfExpression.new(token, condition, consequence, alternative)
    end
    ...
    def parse_block_statement
      ...
      Past::BlockStatement.new(token, statements)
    end
    ...
    def parse_function_literal
      ...
      Past::FunctionLiteral.new(token, parameters, body)
    end
    ...
    def parse_function_parameters
      ...
      next_token
      token = @cur_token
      parameters << Past::Identifier.new(token, token.literal)

      while peek_token_is?(Tokens::COMMA)
        next_token
        next_token
        inner_token = @cur_token
        parameters << Past::Identifier.new(inner_token, inner_token.literal)
      end
    ...
    def parse_hash_literal
      ...
      Past::HashLiteral.new(token, pairs)
    end
